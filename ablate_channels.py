...
def wilson_interval(n_correct, n_total, z=1.96):
    ...
    Byte-for-byte the same function as evaluate_honestly.py lines 94-100, copied
    rather than imported so this file stays runnable on its own -- but copied
    EXACTLY, because an interval computed two different ways is two intervals.
    """
    p = n_correct / n_total
    denom = 1 + z**2 / n_total
...
# COMPLEMENT is derived, never typed. A hand-typed 47-channel list is a list that
# can silently stop being the complement of SENSORIMOTOR the moment either one is
# edited, and then the arm keeps its name while measuring something else. The
# expression below is the same one the hostile pass used at its line 84, so the
# two implementations pick identically or the disagreement is real rather than a
# transcription slip.
