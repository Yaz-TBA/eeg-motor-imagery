...
# The probe. Temporalis territory, and where the fourth retained CSP component
# peaks (T8/T10/TP8).
TEMPORAL = ["T7", "T8", "T9", "T10", "TP7", "TP8", "FT7", "FT8"]

# Deliberately the SAME SIZE as TEMPORAL. ablate_channels.py's own second caveat
# is that its frontopolar row confounds region with an 8x cut in channel count.
# Matching the counts removes that confound from this one comparison. It is also
# where saccadic spike potentials and frontalis EMG are largest.
FRONTOPOLAR = ["Fp1", "Fpz", "Fp2", "AF7", "AF3", "AFz", "AF4", "AF8"]

# Where genuine sensorimotor high-gamma would appear, if it appears at all.
SENSORIMOTOR = [
    "FC3", "FC1", "FCz", "FC2", "FC4",
