...
def make_clf():
    """The committed pipeline. CSP sits INSIDE the Pipeline, so it refits on the
    training fold only, inside every fold, in every replicate. Any other placement
    would leak the test fold into filter estimation and invalidate everything."""
    return Pipeline([
...
def assert_lattice(scores, n, where):
    """Assert 1 and 2. Equal folds mean the fold-mean IS the pooled count over n.
    Off-lattice means unequal folds or a scorer that is not accuracy, and then the
    fold-mean is not the accuracy at all. This is the check that caught two
    arithmetically impossible numbers (95.9%, 47.4%) in an earlier README."""
...
def load_subject(subject):
    ...
    -1.0 to 4.0 s, cropped to 1.0-2.0 s. Also returns the RUN INDEX per epoch,
    computed the way ablate_channels.py:150 computes it. That blocking variable
    has existed in this repo since the ablation rung and has never been used in a
    null; it is the whole of the arm A block correction."""
    mne.set_log_level("ERROR")
    paths = eegbci.load_data(subjects=subject, runs=RUNS, update_path=True)
    raws = [mne.io.read_raw_edf(p, preload=True) for p in paths]
    # Record run lengths BEFORE concatenating: concatenate_raws consumes the list
    # and the run boundary is not recoverable from the result.
    ...
    runs = np.searchsorted(run_edges, onsets, side="right")
